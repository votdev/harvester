...
2. Make sure a second VM without the `harvesterhci.io/maintain-mode-strategy` label is running on the same node. Migrate it to the node on which the other VM is running if necessary.
3. Note the node on which the VMs are running.
4. Go to the `Hosts` page and select the `Enable Maintenance Mode` action in the actions dropdown menu of the node your VMs are running on. Check the `Force` checkbox.
5. After some time the `Cordon` state should be displayed for the host we are putting into maintenance mode.
6. Now go to the `Virtual Machines` page. Both VMs should show the `Stopping` status badge and should be both `Off` after some time.
7. Disable the maintenance mode on the `Hosts` page via the action menu of the node. After some time the `Maintenance Mode` status badge should be gone away.
8. Go to the `Virtual Machines` page. Both VMs should be still off.
9. Choose the `Edit YAML` action menu on the VM that was labeled with `harvesterhci.io/maintain-mode-strategy: ShutdownAndRestartAfterDisable` and make sure the `harvesterhci.io/maintain-mode-strategy-node-name: <NODENAME>` is NOT in the annotations.
